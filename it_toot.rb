...
# Mastodon. If you have a website or blog, you can just run this script every
# time you make a new post and it'll automatically toot about it.
#
...
toot_times_file = @script_dir + "/" + CONFIG['mastodon_cache_file']
...
toot_times = if File.exists?(toot_times_file)
                YAML.load( File.read(toot_times_file) )
            else
...
print "Checking ", feed,  " for tootable posts...", "\n"
...
rss.entries.reverse.each_with_index do|i,idx|
    ...
    #values into the text for a new toot.
p i.updated
p toot_times[feed]
    if i.updated > toot_times[feed]
        ...
        # Notify me via mail that the post was tooted
        message = "Hi #{CONFIG['recipient_name']}, \n\n I just tooted the following toot: \n\n" + text.to_s.force_encoding("utf-8") + ". \n\n Best, \n\n #{CONFIG['from_name']}"
        subject = "New Toot tooted: " + i.title.to_s.force_encoding("utf-8")
